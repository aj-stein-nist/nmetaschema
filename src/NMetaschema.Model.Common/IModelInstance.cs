...
namespace NMetaschema.Model.Common
{
    ...
    {
        //new IAssemblyDefinition getContainingDefinition();
        IAssemblyDefinition getContainingDefinition();

        XmlQualifiedName getXmlGroupAsQName()
        {
            var test = new XmlQualifiedName("test");
            return test;
        }
            //string namespace = getGroupAsXmlNamespace();
            // if(XmlGroupAsBehavior.GROUPED.Equals(getXmlGroupAsBehavior()))
            // {
            //     return new XmlQualifiedName(namespace, getGroupAsName());
            // } else {
